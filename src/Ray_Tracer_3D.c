		    ...
		    Array3D * sino)	//input
{
	...
	for (i = 0; i < NS; i++) {
		...
		for (j = -q; j < q + 1; j++) {
			...
			detector.y = D * sin(j * ss2 + PI + thetaS) + d;
			for (k = -qz; k < qz + 1; k++) {
				if (ARRAY3DData(sino, (j + q), (k + qz), i) <
				    1.e-6) {
					detector.z = k * ssz + d;
...
#endif
					Find_Zero_Pixels(&source, &detector,
							 image);
				}
			}
		}
