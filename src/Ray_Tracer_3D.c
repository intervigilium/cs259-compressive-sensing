                           ...
                           Array3D *image)
{
    ...
    lambda_min = MAX(MAX(0.0, 
                         MIN(LAMBDA_X(0,source->x,detector->x,L),LAMBDA_X(N_x,source->x,detector->x,L))
                        ), 
                     MAX(MIN(LAMBDA_Y(0,source->y,detector->y,L),LAMBDA_Y(N_y,source->y,detector->y,L)), 
                         MIN(LAMBDA_Z(0,source->z,detector->z,L),LAMBDA_Z(N_z,source->z,detector->z,L))
                        )
                     );

    //distance between source and exit point
    lambda_max = MIN(MIN(L, 
                         MAX(LAMBDA_X(0,source->x,detector->x,L),LAMBDA_X(N_x,source->x,detector->x,L))
                        ), 
                     MIN(MAX(LAMBDA_Y(0,source->y,detector->y,L),LAMBDA_Y(N_y,source->y,detector->y,L)), 
                         MAX(LAMBDA_Z(0,source->z,detector->z,L),LAMBDA_Z(N_z,source->z,detector->z,L))
                        )
                     );
    ...
    lambda_0 = lambda_min;
    lambda.x = (absvalue_x < 1e-4)? 1e6 : LAMBDA_X(u.x, source->x, detector->x,L); 
    lambda.y = (absvalue_y < 1e-4)? 1e6 : LAMBDA_Y(u.y, source->y, detector->y,L); 
    lambda.z = (absvalue_z < 1e-4)? 1e6 : LAMBDA_Z(u.z, source->z, detector->z,L); 
...
#ifdef TRACER_FORWARD_DEBUG
{
   ...
   fclose(outputfile);
                           ...
                           //if(i==11 && k ==-101)
                           {
                             aa = Ray_Tracer_Forward(&source, &detector, N_x, N_y, N_z,image);
                           //  printf("i=%d, j=%d, k=%d\n", i, j, k);
                           }
                         ...
                         Array3D *image_denote)
{
    ...
    lambda.x = (absvalue_x < 1e-6)? 1e6 : LAMBDA_X(u.x, source->x, detector->x, L); 
    lambda.y = (absvalue_y < 1e-6)? 1e6 : LAMBDA_Y(u.y, source->y, detector->y, L); 
    lambda.z = (absvalue_z < 1e-6)? 1e6 : LAMBDA_Z(u.z, source->z, detector->z, L); 
		...
		if (index == 1)
		{
			 ...
			 if (lambda.z < lambda.y )
			 {
				 ...
				 if (Tz < Tx)
				 {
					 ...
					 {
						 length = Tz;
                                                 ARRAY3DData(image, v.x, v.y, v.z) += length*value;
	...
	for (i = 0; i <= NS; i++)
	{
		...
		for (j = -q; j < q + 1; j++)
		{
			...
			for (k = -qz; k < qz + 1; k++)
			{
                            ...
                            if ( value > 1.e-6)
                              Ray_Tracer_Backward(&source, &detector, N_x, N_y, N_z, value, image, image_denote);
			}
		}
		thetaS += dtheta2;
...
void Find_Zero_Pixels(Vector3D *source, Vector3D *detector, Array3D *image)
{
    ...
    lambda.x = (DataType)((absvalue_x < 1e-6)? 1e6 : LAMBDA_X(u.x, source->x, detector->x, L)); 
    lambda.y = (DataType)((absvalue_y < 1e-6)? 1e6 : LAMBDA_Y(u.y, source->y, detector->y, L)); 
    lambda.z = (DataType)((absvalue_z < 1e-6)? 1e6 : LAMBDA_Z(u.z, source->z, detector->z, L)); 
...
{
   FILE *outputfile;
   outputfile = fopen("initialization_source.dat", "a");
   if ( outputfile == NULL)
   {
     printf("Cannot open the output file!\n");
     exit(1);
    ...
    fprintf(outputfile,"%lf\n",source.z);
   fclose(outputfile);
   outputfile = fopen("initialization_detector.dat", "a");
   if ( outputfile == NULL)
   {
     printf("Cannot open the output file!\n");
     exit(1);
