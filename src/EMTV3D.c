...
int main(int argc, char** argv)
{
	...
	if(configure(argc, argv)==-1)
        ...
        Array3D_initialize_value ((cpu_image_denote.dataPtr), 1.0, N_x, N_y, N_z);
        Array3D_malloc(&cpu_sino, 2*q + 1, 2*qz + 1, (int)floorf(359./theta)+1);
        Array3D_initialize_value((cpu_sino.dataPtr), 1.0, 2*q + 1, 2*qz + 1, (int)floorf(359./theta)+1);
