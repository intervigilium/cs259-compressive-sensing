		...
		for (j = 1; j < N_x; j++)
			for (k = 1; k < N_y; k++)
				for (l = 1; l < N_z; l++) {
					...
					if (data000 > 1e-6) {
						data100 =
						    ARRAY3DData(image, (j + 1),
								k, l);
						data010 =
						    ARRAY3DData(image, j,
								(k + 1), l);
						data001 =
						    ARRAY3DData(image, j, k,
								(l + 1));
						datan100 =
						    ARRAY3DData(image, (j - 1),
								k, l);
						...
						temp =
						    ...
						    (ARRAY3DData
						     ...
						     ARRAY3DData(image, j, k,
								 (l + 1)))
						    + c2 * ARRAY3DData(image,
								       (j - 1),
								       k, l)
						    + c3 * ARRAY3DData(image, j,
								       (k - 1),
								       l)
						    + c4 * ARRAY3DData(image, j,
								       k,
								       (l - 1));
