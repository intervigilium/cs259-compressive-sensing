			...
			for (k = 1; k < N_y; k ++)
				for (l = 1; l < N_z; l ++)
				{
                                     ...
                                     temp = alpha* ARRAY3DData(image, j, k, l) 
                                       + c1 *(ARRAY3DData(image,(j+1), k, l) + ARRAY3DData(image, j,(k+1), l) + ARRAY3DData(image, j, k,(l+1))) 
                                       + c2 * ARRAY3DData(image,(j-1), k, l) 
                                       + c3 * ARRAY3DData(image, j, (k-1), l) 
                                       + c4 * ARRAY3DData(image, j, k, (l-1));

                                      ARRAY3DData(image, j, k, l) = (DataType)(temp / (alpha + 3.0 * c1 + c2 + c3 + c4)); 
                                     }
/*
if (image(j,k,l) > 1e-8)
{
...
*/
		...
		for (j = 1; j < N_x; j ++)
			for (k = 1; k < N_y; k ++)
			{
                            ...
                            ARRAY3DData(image, j, k, N_z) = ARRAY3DData(image, j, k, (N_z -1));
			}
	}
}
