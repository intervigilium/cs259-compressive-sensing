	...
	for (i = 0; i < max_iter; i ++)
	{
		...
		for (j = 0; j < M_x; j ++)
			for (k = 0; k < M_y; k ++)
				for (l = 0; l < M_z; l ++)
					...
					{
					   ARRAY3DData((&gAf), j,k,l) = ARRAY3DData(sino, j,k,l)/ ARRAY3DData((&Af),j,k,l);
					}
	    
		DBG1(fprintf(stderr, "\n");)

		Backward_Projection(&AtAf, image_denote, D, q, ss, d, dtheta, ssz, qz, &gAf);
		...
		for (j = 0; j < N_x; j ++)
			for (k = 0; k < N_y; k ++)
				for (l = 0; l < N_z; l ++)
					{
					  ARRAY3DData(image,j,k,l) = ARRAY3DData(image,j,k,l) *ARRAY3DData((&AtAf), j,k,l)/ARRAY3DData(sumA,j,k,l);
					}
	}
